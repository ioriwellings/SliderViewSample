 ...
 * Pager Indicator. 圆点
 */
public class SliderPagerIndicator extends LinearLayout implements
	...
	/**
	 * This value is from
	 * {@link com.marshalchen.common.uimodule.slider.SliderAdapter}
	...
	private Handler handler = new Handler() {
		public void handleMessage(Message msg) {
			if (mPager != null) {
				Log.i("===", "handleMessage : " + currentItem);
	...
	public SliderPagerIndicator(Context context, AttributeSet attrs) {
		super(context, attrs);

		mContext = context;

		final TypedArray attributes = context.obtainStyledAttributes(attrs,
				R.styleable.SliderPagerIndicator, 0, 0);

		int visibility = attributes.getInt(
				R.styleable.SliderPagerIndicator_visibility,
		...
		int shape = attributes.getInt(R.styleable.SliderPagerIndicator_shapes,
				Shape.Oval.ordinal());
		...
		mUserSetSelectedIndicatorResId = attributes.getResourceId(
				R.styleable.SliderPagerIndicator_selected_drawable, 0);
		mUserSetUnSelectedIndicatorResId = attributes.getResourceId(
				R.styleable.SliderPagerIndicator_unselected_drawable, 0);

		mDefaultSelectedColor = attributes.getColor(
				R.styleable.SliderPagerIndicator_selected_color,// 204, 142, 59
				Color.rgb(204, 142, 59));
		mDefaultUnSelectedColor = attributes.getColor(
				R.styleable.SliderPagerIndicator_unselected_color,// 72, 55, 35
				Color.rgb(72, 55, 35));

		mDefaultSelectedWidth = attributes.getDimension(
				R.styleable.SliderPagerIndicator_selected_width,
				(int) pxFromDp(6));
		mDefaultSelectedHeight = attributes.getDimensionPixelSize(
				R.styleable.SliderPagerIndicator_selected_height,
				(int) pxFromDp(6));

		mDefaultUnSelectedWidth = attributes.getDimensionPixelSize(
				R.styleable.SliderPagerIndicator_unselected_width,
				(int) pxFromDp(6));
		mDefaultUnSelectedHeight = attributes.getDimensionPixelSize(
				R.styleable.SliderPagerIndicator_unselected_height,
				(int) pxFromDp(6));
		...
		mPadding_left = attributes.getDimensionPixelSize(
				R.styleable.SliderPagerIndicator_padding_left,
				(int) pxFromDp(3));
		mPadding_right = attributes.getDimensionPixelSize(
				R.styleable.SliderPagerIndicator_padding_right,
				(int) pxFromDp(3));
		mPadding_top = attributes
				.getDimensionPixelSize(
						R.styleable.SliderPagerIndicator_padding_top,
						(int) pxFromDp(0));
		mPadding_bottom = attributes.getDimensionPixelSize(
				R.styleable.SliderPagerIndicator_padding_bottom,
				(int) pxFromDp(0));

		mSelectedPadding_Left = attributes.getDimensionPixelSize(
				R.styleable.SliderPagerIndicator_selected_padding_left,
				(int) mPadding_left);
		mSelectedPadding_Right = attributes.getDimensionPixelSize(
				R.styleable.SliderPagerIndicator_selected_padding_right,
				(int) mPadding_right);
		mSelectedPadding_Top = attributes.getDimensionPixelSize(
				R.styleable.SliderPagerIndicator_selected_padding_top,
				(int) mPadding_top);
		mSelectedPadding_Bottom = attributes.getDimensionPixelSize(
				R.styleable.SliderPagerIndicator_selected_padding_bottom,
				(int) mPadding_bottom);

		mUnSelectedPadding_Left = attributes.getDimensionPixelSize(
				R.styleable.SliderPagerIndicator_unselected_padding_left,
				(int) mPadding_left);
		mUnSelectedPadding_Right = attributes.getDimensionPixelSize(
				R.styleable.SliderPagerIndicator_unselected_padding_right,
				(int) mPadding_right);
		mUnSelectedPadding_Top = attributes.getDimensionPixelSize(
				R.styleable.SliderPagerIndicator_unselected_padding_top,
				(int) mPadding_top);
		mUnSelectedPadding_Bottom = attributes.getDimensionPixelSize(
				R.styleable.SliderPagerIndicator_unselected_padding_bottom,
				(int) mPadding_bottom);
	...
	public void setViewPager(ViewPager pager) {
		...
		SliderAdapter adapter = (SliderAdapter) mPager.getAdapter();
	...
	private DataSetObserver dataChangeObserver = new DataSetObserver() {
		...
		public void onChanged() {
			...
			mItemCount = count;
			setItemAsSelected(mPager.getCurrentItem());
		}
