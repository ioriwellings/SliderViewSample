 ...
 * {@link com.soft.slider.widget.Transformers.marshalchen.common.uimodule.slider.Tricks.ViewPagerEx.PageTransformer}
...
public abstract class BaseTransformer implements ViewPager.PageTransformer {
	 ...
	 * {@link android.view.View#GONE}. Returning false will force the fragment
	 * to {@link android.view.View#VISIBLE}.
	 * 
	 * @return
	 */
	 ...
	 * {@link #onTransform(android.view.View, float)} is called.
	 * 
	 * @param view
	 * @param position
	 */
	protected void onPreTransform(View view, float position) {
		...
		ViewHelper.setTranslationX(view, isPagingEnabled() ? 0f : -width
				* position);

		if (hideOffscreenPages()) {
			ViewHelper.setAlpha(view, position <= -1f || position >= 1f ? 0f
					: 1f);
		} else {
						...
						if (zero > 0) {
							if (cha > -1 && cha < 0) {
								...
								mCustomAnimationInterface
										.onPrepareNextItemShowInScreen(view);
							} else {
								// out
								mCustomAnimationInterface
										.onPrepareCurrentItemLeaveScreen(view);
							}
						} else {
							if (cha > -1 && cha < 0) {
								// out
	...
	/**
	 * Called each {@link #transformPage(android.view.View, float)} call after
	 * {@link #onTransform(android.view.View, float)} is finished.
	 * 
	 * @param view
	 * @param position
	 */
	...
	public void setCustomAnimationInterface(
			BaseAnimationInterface animationInterface) {
